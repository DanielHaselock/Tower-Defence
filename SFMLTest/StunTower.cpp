...
#include "StunTower.h"
using namespace TD;
void StunTower::Update(TD::EnemyArmy& EnemyArmy, Renderer& my_renderer, float Deltatime, Player& player)
{
    ...
    if (EnemyArmy.GetArmy()[index]->is_dead != true)
    {
        ...
        if (EnemyArmy.GetArmy()[index] != nullptr)
        {
            ...
            {
                EnemyArmy.RemoveEnemy(*EnemyArmy.GetArmy()[index]);
                return;
            }
        }
    }
...
StunTower::StunTower(sf::Vector2i cellPosition)
{
...
void StunTower::ShootAt(int index, Renderer& my_renderer, float b, float Deltatime)
{
